...
from pyumls.sqlite import SQLite			# for py-umls standalone
...
class UMLS (object):
	...
	def check_database(cls):
		...
		umls_db = os.path.join('pyumls', 'databases', 'umls.db')
		if not os.path.exists(umls_db):
			raise Exception("The UMLS database at {} does not exist. Run the import script `pyumls/databases/umls.sh`."
...
class UMLSLookup (object):
	...
	sqlite = None
	did_check_dbs = False
	preferred_sources = ['"RXNORM"','"SNOMEDCT"','"MTH"']
	
	def __init__(self):
		absolute = os.path.dirname(os.path.realpath(__file__))
		self.sqlite = SQLite.get(os.path.join(absolute, 'pyumls/databases/umls.db'))
...
if '__main__' == __name__:
    ...
    with open("../data/extended/preprocessing/NDCmappings/ndc_listofcuis.txt","r") as file:
        codeList = file.readlines()
    
    codeTextMapping = dict()
    for code in codeList:
        code = code.strip("\n")
        
## look.lookup_code(code) returns a list of matches like "[('Albuterol 0.833 MG/ML / Ipratropium Bromide 0.167 MG/ML Inhalation Solution', 'MTH', 'T200')]" so we need to access the item in the list, and then the first position of the tuple which is where we find the descriptive string
        descriptiveString = look.lookup_code(code)[0][0]
        codeTextMapping[code] = descriptiveString
        
    with open("../data/extended/preprocessing/NDCmappings/cui_text.json","w") as file:
        json.dump(codeTextMapping, file)
