...
from modeling_diagnosis import BertForSequenceClassification
from data_processor_precomputation_diagnosis import convert_examples_to_features, processorNoText
...
def main():
    ...
    for i in range(numFolds):
        ...
        with torch.no_grad():
            test_batch_size = 1
            filename = "fold" + str(i) + "_allvisits_diagnosis_notext.csv"
            filepath = os.path.join(args.data_dir, filename)
            examples = processor.get_examples(filepath, LabelToPredict, features=args.features, fold=i)

            print("***** Loading Fold {} *****".format(i))
            print("  Num examples = %d", len(examples))
            features = convert_examples_to_features(examples, args.features, maxLenDict)
            ...
            if "small_diag_icd9"  in args.features:
                tensors.append(torch.tensor([f.small_diag_icd9 for f in features], dtype=torch.long))
                featurePositionDict["small_diag_icd9"] = positionIdx
                positionIdx+=1
            if "diag_ccs"  in args.features:
                tensors.append(torch.tensor([f.diag_ccs for f in features], dtype=torch.long))
            ...
            array_filepath = "../../data/extended_folds/discharge_subjectsplit_trajectory" + "/fold" + str(i) + "_allvisits_diagnosis_ccs_precomputed.npy"
